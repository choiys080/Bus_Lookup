...
Write-Host "Authenticated anonymously. Token acquired."
...
if ($queryResponse -and $queryResponse.document) {
    foreach ($item in $queryResponse) {
        if ($item.document) {
            ...
            # docPath is usually "projects/projectId/databases/(default)/documents/..."
            # We need to call PATCH on https://firestore.googleapis.com/v1/{name}
